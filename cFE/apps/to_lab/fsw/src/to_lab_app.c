...
void Setup_OpenFlow_TO_Connection(char *address, int port)
{
	struct sockaddr_in serv_addr;
	
	if ((to_openflowSock = socket(AF_INET, SOCK_STREAM, 0)) < 0)
	{
	...
	if (connect(to_openflowSock, (struct sockaddr *) &serv_addr, sizeof(serv_addr)) < 0)
	{
		printf("ERROR: TO Connection Failed\n");
		return;
	}
}

void TO_output_openflow_packet(char *msg_to_output)
{
    /* initialize data types packet */
    CFE_SB_InitMsg(&openflow_pkt,
                   TO_LAB_OPENFLOW_MID,
                   sizeof(openflow_pkt), TRUE);

    CFE_SB_TimeStampMsg((CFE_SB_MsgPtr_t) &openflow_pkt);

    /* initialize the packet data */
    memcpy(openflow_pkt.ofMsg, msg_to_output, TO_OF_MSG_SIZE);
...
void Forward_OpenFlow_Messages(void)
{
	...
	int bytes_read = 0;
	bytes_read = read(to_openflowSock, msg_buf, TO_OF_MSG_SIZE);
	if (bytes_read > 0)
	{
		TO_output_openflow_packet(msg_buf);
	}
}
...
{
   Setup_OpenFlow_TO_Connection("127.0.0.1", 1236);
