...
void Setup_OpenFlow_CI_Connection(char *address, int port)
{
	struct sockaddr_in serv_addr;
	
	if ((ci_openflowSock = socket(AF_INET, SOCK_STREAM, 0)) < 0)
	{
	...
	if (connect(ci_openflowSock, (struct sockaddr *) &serv_addr, sizeof(serv_addr)) < 0)
	{
		printf("ERROR: CI Connection Failed\n");
		return;
	}
}

void Send_OpenFlow_Message(char *message)
{
	send(ci_openflowSock, message, strlen(message), 0);
}
...
{
    Setup_OpenFlow_CI_Connection("127.0.0.1", 1235);
    int32  status;
...
void CI_TaskInit(void)
{
    ...
    CFE_SB_Subscribe(CI_LAB_OPENFLOW_MID, CI_CommandPipe);
...
void CI_ProcessCommandPacket(void)
{
    ...
    switch (MsgId)
    {
        ...
        case CI_LAB_OPENFLOW_MID:
            CI_ProcessOpenFlowCommand();
            break;
...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * **/
/*                                                                            */
/* CI_ProcessOpenFlowCommand() -- CI OpenFlow commands                        */
/*                                                                            */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * **/

void CI_ProcessOpenFlowCommand(void)
{
    int len = CFE_SB_GetUserDataLength(CIMsgPtr);
    char *ptr = CFE_SB_GetUserData(CIMsgPtr);
    char buf[256];
    buf[len] = '\0';
    memcpy(buf, ptr, len);
    Send_OpenFlow_Message(ptr);
    
    return;

} /* End of CI_ProcessOpenFlowCommand() */
